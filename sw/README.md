...
# You can now flash it by holding down the BOOT button and resetting the device, then mounting the usb mass storage device, then copy the file openpico64/openpico64.uf2 to the mounted path and run `sync`.
...
openocd -f interface/jlink.cfg -c "transport select swd; adapter_khz 1000" -f target/rp2040.cfg -c "program ./openpico64/openpico64.elf verify reset exit"
